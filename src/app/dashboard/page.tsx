...
import { listBookings, getBookingStats, getServicePopularity, getVehicleDistribution } from "@/db/queries";
import {
  mockMonthlyCosts,
  mockCostBreakdown,
  mockProfitMargins,
} from "@/data/mock-dashboard";
import type { BookingRow, CustomerAnalyticsData } from "@/components/dashboard/CustomerAnalytics";
...
export default async function DashboardPage() {
  ...
  const [rawBookings, stats, servicePopularity, vehicleDistribution] = await Promise.all([
    listBookings(50),
    getBookingStats(),
  ...
  const customerData: CustomerAnalyticsData = {
    ...
    servicePopularity: servicePopularity.map((r) => ({ label: r.label, value: Number(r.value) })),
