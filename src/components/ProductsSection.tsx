...
const PRICE_RANGES: { id: PriceRangeId; label: string; test: (p: number) => boolean }[] = [
  { id: "all", label: "Any price", test: () => true },
  { id: "under-25", label: "Under $25", test: (p) => p < 25 },
  { id: "25-50", label: "$25 – $50", test: (p) => p >= 25 && p <= 50 },
  { id: "over-50", label: "Over $50", test: (p) => p > 50 },
];

const badgeVariantFor = (badge: ProductBadge) =>
  badge === "Best seller" ? "default" : "outline";
...
const ProductsSection = () => {
  ...
  const filtered = useMemo<Product[]>(() => {
    ...
    const priceTest = PRICE_RANGES.find((r) => r.id === priceRange)!.test;

    return products.filter((p) => {
      if (selectedCategories.size > 0 && !selectedCategories.has(p.category)) return false;
      if (!priceTest(p.price)) return false;
      if (query) {
        const haystack = `${p.name} ${p.tagline} ${CATEGORY_LABEL[p.category]}`.toLowerCase();
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="flex flex-col gap-7 rounded-[20px] border border-line bg-card-gradient p-6">
                  ...
                  >
                    <div className="relative aspect-[16/10] w-full overflow-hidden bg-secondary">
                      ...
                      <div className="absolute inset-x-4 bottom-4 flex items-center justify-between gap-2">
                        ...
                        {product.badge && (
                          <Badge
                            variant={badgeVariantFor(product.badge)}
                            className={
                              badgeVariantFor(product.badge) === "outline"
