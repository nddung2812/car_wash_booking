...
5. **Email delivery (`src/lib/booking-notify.ts`)** — `deliverBookingEmails()` is the only way booking emails are sent. It fires **both** halves in parallel: the internal notification to the business inbox (`BOOKING_NOTIFICATION_EMAIL`, else first `ADMIN_EMAILS`) and the customer's own copy (via `recipientOverride`). It **must always be awaited** — it was previously `void`-ed in the POST route, so Vercel froze the function before the request reached EmailJS and 25 of 32 bookings between June and September 2026 silently sent nothing. Guarantees: three retries with backoff per send; never throws, so a saved booking survives a total email outage; and each half stamps `bookings.admin_email_sent_at` / `customer_email_sent_at` on success, so a failure is a visible null rather than a lost email. `scripts/resend-booking-emails.ts --missing` sweeps those nulls (idempotent; withholds the customer copy for past-dated bookings unless `--include-past`).
6. `{{payment_status}}` strings live in `booking-payment.ts` (`PAY_AT_COLLECTION_STATUS`, `PAY_NOW_PAID_STATUS`, `depositPaidStatus()`). Every flow goes `deliverBookingEmails()` → `sendBookingNotification()` in `src/lib/email.ts`; the EmailJS booking template must include `{{payment_status}}`, and optionally `{{amount_paid}}` / `{{balance_due}}` which are now passed too (EmailJS ignores params a template doesn't reference).
