...
2. `POST /api/bookings` — validates, calculates price, generates confirmation code, saves booking with chosen payment method
   - **Pay at collection** → **awaits** `deliverBookingEmails()` with `{{payment_status}} = "Pay later - At collection"`, returns `redirectUrl: /success?code=…`
